...
def lambda_handler(event, context):
    ...
    # 파일 이름(경로지정)
    filename = '_'.join(global_data["data"]) + "X"  + '_'.join(global_data["date"])+ "X"+ "drawing1" 
    
    try:
        s3.head_object(Bucket=target_bucket_name, Key=f'{filename}.html')
        print('버킷에 파일 있음 그거 가져가면됨') 
